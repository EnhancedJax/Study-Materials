...
\begin{definition}
    ...
    The \textbf{average service time} (when the server is idle) is given by:
    \[S = \frac{L}{R}\]

    Where $L$ is the packet length (bits) and $R$ is the transmission rate (bps).
\end{definition}
...
\begin{definition}
    ...
    The \textbf{traffic intensity / server utilization} measures how busy a link is:

    \[U = \frac{L \times a}{R} = S \times a\]

    Where $a$ is the average packet arrival rate.
    ...
    \[L = 500 \times 8 = 4000 \text{ bits}, a = 100 \text{ packets/sec}, R = 1 \times 10^6 \text{ bps}, U = 0.4\]
\end{definition}
...
\begin{definition}
    ...
    Waiting time to get access to the link.
    \begin{itemize}
        \item Each link has a queue where packets wait to be transmitted.
        \item The higher the \textbf{traffic intensity}, the link is busier, and higher probability that there is one or more packets in the queue.
    \end{itemize}
