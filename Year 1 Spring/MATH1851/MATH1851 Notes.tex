...
\begin{theorem}
    ...
    WARNING: If the \textbf{limit is infinity} it \textbf{doesn't exist}.
...
\begin{theorem}
    {Rolle's theorem}
    For $f(x)$ that is \emph{continuous} in $[a,b]$ and \emph{differentiable} in $(a,b)$, if $f(a)=f(b)$, there exists $c\in(a,b): f'(c)=0$
    \tcblower
...
\begin{definition}
    {Max/minimum points}
    The \textbf{absolute maximum/minimum points} are the points where the function has the largest/smallest value in the entire domain. It can be written as:
...
\begin{table}[H]
    \begin{tabular}{rll}
        ...
        4. & Critical points                      & $f'(x)=0 \Rightarrow f''(x)$ max or min ($+f''(x)$ or $-f''(x)$) \\
        5. & Inflection points                    & $f''(x)=0$                                                       \\
        6. & Horizontal asymptotes                & $\lim_{x\to\pm\infty}f(x)=n?$                                    \\
        7. & Vertical asymptotes                  & $\lim_{x\to a^\pm}f(x)=\pm\infty?$                               \\
        8. & Area strictly increasing/decreasing? & ($f'(x)>0$ or $f'(x)<0$)                                         \\
        9. & Area concavity?                      & ($f''(x)>0$ or $f''(x)<0$)                                       \\
    \end{tabular}
\end{table}
