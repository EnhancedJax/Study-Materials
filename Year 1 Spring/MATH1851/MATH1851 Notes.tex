...
\begin{theorem}
    {Volume by disk}
    Used to find volume by rotating a \textbf{curve} around an axis.

    Volume by x-axis: ($y:x\to y$)
    \[V=\pi\int_{a}^{b}f^2(x)dx\]
    Where $f(x)$ is the function of the curve, and $a,b$ are the limits of the curve.
\end{theorem}
\begin{theorem}
    {Volume by Washers}
    Used to find volume by rotating a \textbf{region bounded by 2 curves} around an axis.

    Volume by x-axis ($y:x\to y$)
    \[V=\pi\int_{a}^{b}R^2(x)-r^2(x)\:dx\]
    Where $R(x)$ is the outer curve, $r(x)$ is the inner curve.
\end{theorem}
\begin{theorem}
    {Volume by Cylindrical shells}
    Used to find volume by rotating a \textbf{curve} around a \textbf{vertical line} $x=n$.

    Volume is given by:
    \[V=2\pi\int_{a}^{b}(x-n)f(x)dx\]
    Where $n$ is the vertical line.
\end{theorem}
...
\begin{theorem}
    ...
    Surface area by x-axis ($y:x\to y$):
    \[A=2\pi\int_{a}^{b}f(x)\sqrt{1+f'^2(x)}dx\]
    And for parametized equations $x=f(t)$, $y=g(t)$:
