...
\begin{table}[H]
    \begin{tabular}{ll}
        ...
        \lstinline|sort -u -k#,# -k#,# ... <file>| & \begin{tabular}{@{}l@{}}Return \textbf{unique rows by column}\\\Colorbox{superlightgray}{\lstinline|-k#,#|} Column \#number,\#number to check unique by\end{tabular}                                                                                                                                                                                   \\ \arrayrulecolor{lightgray}\hline
...
\begin{knBox}
    ...
    \lstinputlisting[language=bash, firstline=0, lastline=4]{code/helloworld.sh}
\end{knBox}
...
\lstinputlisting[language=bash, firstline=6, lastline=14]{code/helloworld.sh}
...
\lstinputlisting[language=bash, firstline=16, lastline=22]{code/helloworld.sh}
...
\lstinputlisting[language=c++, firstline=0, lastline=9]{code/cpp.cpp}
\begin{enumerate}
...
A header file is a file with the extension \Colorbox{superlightgray}{\lstinline|.h|} that contains the function declarations. The implementation is in the \Colorbox{superlightgray}{\lstinline|.cpp|} file \emph{with the same name}.

A simple header file looks like this. ("header" is a placeholder name)
\lstinputlisting[language=c++, lastline=7]{code/header.cpp}
We then can use the header file and the functions defined in \Colorbox{superlightgray}{\lstinline|header.cpp|} in another program by including it:
\lstinputlisting[language=c++, firstline=9, lastline=9]{code/header.cpp}

\subsection{Makefile}
A \Colorbox{superlightgray}{\lstinline|Makefile|} is a file that contains a set of rules used to build a target program. The file must be named as \Colorbox{superlightgray}{\lstinline|Makefile|}.

\begin{itemize}
    \item \Colorbox{superlightgray}{\lstinline|.o|} files are object files, which are the result of compiling the source files.
    \item \Colorbox{superlightgray}{\lstinline|g++|} is the C++ compiler
    \item The \Colorbox{superlightgray}{\lstinline|-c|} flag asks to compile the file into an .o file, but not to link it into an executable. The absence of it will compile and link the file.
    \item The \Colorbox{superlightgray}{\lstinline|-o|} flag asks to name the output file
\end{itemize}

Here's a complete example of \Colorbox{superlightgray}{\lstinline|makefile|}, all you need to know is explained by the comments:
\lstinputlisting[language=make]{code/makefile}
