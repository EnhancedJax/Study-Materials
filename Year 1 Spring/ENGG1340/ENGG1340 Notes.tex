...
A header file is a file with the extension \Colorbox{superlightgray}{\lstinline|.h|} that contains the function declarations. The implementation is in the \Colorbox{superlightgray}{\lstinline|.cpp|} file. 

A simple header file looks like this:
\lstinputlisting[language=c++]{code/header.cpp}
...
\Colorbox{superlightgray}{\lstinline|makefile|} is used for describing the dependency of files for the \Colorbox{superlightgray}{\lstinline|make|} tool which smartly recompiles only the files that have changed.
The file name is important, it must be named \Colorbox{superlightgray}{\lstinline|makefile|} or \Colorbox{superlightgray}{\lstinline|Makefile|} with no extension.
