...
\end{figure}
This will also be useful in section "\hyperref[sec:statics]{Statics and restraints}".
...
\begin{definition}
    ...
    \[\epsilon=\frac{\Delta L}{L}\quad\text{(Ratio)}\]
    Where $\Delta L$ is the change in length, and $L$ is the original length of the member.
\end{definition}
...
\begin{definition}
    ...
    The water pressure acting on any surface is \emph{always perpendicular} to the surface, and the water pressure in \emph{any directions is the same}. The pressure is given by:
    \[p=\rho gh\quad(Pa)\]
...
\label{sec:statics}
\section{Statics and restraints}
\subsection{Statics}
\begin{definition}
    {Static equilibrium}
    System in \textbf{Static equilibrium} experiences no acceleration when loads are applied to it.
\end{definition}
\begin{knBox}
    ...
    \begin{table}[H]
        ...
        \end{tabular}
    \end{table}
    Refer to section "\hyperref[sec:support]{support reactions}" for details.
\end{knBox}
...
\begin{definition}
    ...
    \end{enumerate}
    Static systems can be further classified into \emph{statically determinate} and \emph{statically indeterminate}.

    \begin{table}[H]
        \begin{tabular}{llll}
            \textbf{Restraints} & \textbf{Support reactions} & \textbf{Number of unknowns} & System is statically   \\ \hline
            Just enough         & All solvable               & no. of equations            & \textbf{determinate}   \\ \arrayrulecolor{lightgray}\hline
            More than enough    & Unsolvable                 & $>$ no. of equations        & \textbf{indeterminate} \\ \arrayrulecolor{lightgray}\hline
        \end{tabular}
    \end{table}
    \begin{center}
        If there are \textbf{3 reaction forces}, the system is statically determinate. (3 equilibriums)
        \includegraphics[width=0.4\textwidth]{./img/3react.jpg}
    \end{center}
\end{definition}
\begin{theorem}
    {Degree of indeterminacy}
    The degree of indeterminacy(or redundancies) is simply $R-3$ where $R$ is the number of reaction forces.
    \tcblower
    \begin{center}
...
\end{knBox}
\begin{theorem}
    {Resultant force and moment for distributed load}
    Intuitively, the resultant force is to simply convert the units to $N$ by $F\times area$.
    \begin{itemize}
        \item Unifrom $\to$ $F=wl$
        \item Triangular $\to$ $F=\frac{1}{2}wl$
    \end{itemize}

    For $M=Fd$, $d$ is given by the distance from the \textbf{origin to the centroid} of the distributed load. The following gives the centroid for some common loads:
    \begin{itemize}
        \item Uniform $\to$ \textbf{middle} of the load.
        \item Trianglular $\to$ \textbf{one third from the base} of the load.
    \end{itemize}
\end{theorem}
...
\subsubsection{Types of trusses}
A quadrangular truss is a mechanism, and a triangular truss is a statically determinate structure.
\begin{center}
    \includegraphics[width=0.45\textwidth]{./img/quadtruss.png}
    \includegraphics[width=0.2\textwidth]{./img/tritruss.png}
\end{center}

\subsubsection{Statically determinate trusses}
...
\subsubsection{Analysing trusses}
We make the following assumptions when analysing trusses:
\begin{enumerate}
    \item The loads are \emph{applied} at the joints only.
    \item The members experience \emph{little deformation}.
    \item The members are \emph{connected} connected with \emph{frictionless pins}.
\end{enumerate}
\begin{knBox}
...
\begin{theorem}
    {Method of joints}
    To find the forces on the members of a truss, refer to the following steps:
    \begin{enumerate}
        \item Isolate a \textbf{solvable joint}.
        \item Draw the FBD of the solvable joint by \textbf{replacing members with forces}.
        \item Use $\sum F_x=0=\sum F_y$ to solve for the unknown forces.
    \end{enumerate}
    Keep in mind that \emph{the joint must be in equilibrium}. That is, the \textbf{external forces} are balanced by \textbf{internal forces}.
    \tcblower
    Consider the following truss system. The only solvable join is joint B. After drawing the FBD on joint B, we can solve for the forces on the members.
    \begin{center}
        \includegraphics[width=0.4\textwidth]{./img/trussFBD.jpg}
        $\to$
        \includegraphics[width=0.4\textwidth]{./img/jointFBD.jpg}
    \end{center}
    \begin{align*}
        500    & =F_{BC}\sin 45^\circ \\
        F_{BC} & =707.1N
    \end{align*}
    Repeating these steps on the other joints will give us the forces on the members.
\end{theorem}
\begin{theorem}
    {Method of sections}
    We cam divide the truss into sections by cutting through the selected members and analyzing the section as a rigid body.

    This is used to solve for the unknown forces within specific members of a truss \emph{without solving for them all}.

    The \emph{method of joints} is simply a special case of the \emph{method of sections}.

\end{theorem}

\begin{knBox}
    {Tension vs. compression}
    By convention, we always draw:
    \begin{itemize}
        \item Tension forces $\to$ \textbf{pointing away} from the joint
        \item Compression forces $\to$ \textbf{pointing towards} the joint
    \end{itemize}
\end{knBox}
\begin{theorem}
    {Zero-force members}
    ...
    \end{itemize}
    Removing zero-force members allows us to simplify the truss.
    \tcblower
    \begin{center}
